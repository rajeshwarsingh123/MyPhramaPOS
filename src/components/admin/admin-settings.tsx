      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <SettingsSectionCard
            ...
            iconBg="bg-purple-500/15"
            isLoading={isLoading}
            hasChanges={sectionHasChanges('plan')}
            isSaving={saveSectionMutation.isPending}
            onReset={() => resetSection('plan')}
            onSave={() => saveSectionMutation.mutate('plan')}
          >
            {/* Free Plan Features */}
            <div>
              <div className="flex items-center gap-2 mb-1">
                <Label className="text-white/70 text-sm">Free Plan Features</Label>
                <Badge className="bg-gray-500/20 text-gray-300 border-gray-500/30 text-[10px]">
                  Free
                </Badge>
              </div>
              <p className="text-[11px] text-white/40 mb-3">
                Select features available on the free plan
              </p>
              <div className="grid grid-cols-2 gap-2">
                {FREE_FEATURE_OPTIONS.map((feat) => (
                  <label
                    key={feat.id}
                    className={cn(
                      'flex items-center gap-2 p-2.5 rounded-lg border cursor-pointer transition-colors',
                      freeFeatures.includes(feat.id)
                        ? 'bg-purple-500/10 border-purple-500/25'
                        : 'bg-white/[0.02] border-white/5 hover:bg-white/[0.04]',
                    )}
                  >
                    <Checkbox
                      checked={freeFeatures.includes(feat.id)}
                      onCheckedChange={() => toggleFreeFeature(feat.id)}
                      className="data-[state=checked]:bg-purple-600 data-[state=checked]:border-purple-600"
                    />
                    <span
                      className={cn(
                        'text-xs',
                        freeFeatures.includes(feat.id) ? 'text-white/90' : 'text-white/50',
                      )}
                    >
                      {feat.label}
                    </span>
            ...
            <div>
              <div className="flex items-center gap-2 mb-1">
                <Label className="text-white/70 text-sm">Pro Plan Pricing</Label>
                <Badge className="bg-purple-500/20 text-purple-300 border-purple-500/30 text-[10px]">
                  Pro
                </Badge>
              </div>
              <p className="text-[11px] text-white/40 mb-3">Set subscription pricing for the Pro plan</p>
              <div className="grid grid-cols-2 gap-3">
                <FieldRow label="Monthly" description="">
                  <div className="relative">
                    ...
                    <Input
                      type="number"
                      value={s.pro_plan_price_monthly}
                      onChange={(e) => updateField('pro_plan_price_monthly', e.target.value)}
                      className="pl-7 bg-[oklch(0.14_0.02_250)] border-[oklch(0.28_0.03_250)] text-white"
                    />
                  </div>
                </FieldRow>
                <FieldRow label="Yearly" description="">
                  <div className="relative">
                    <span className="absolute left-3 top-1/2 -translate-y-1/2 text-white/40 text-sm">
                      ₹
                    </span>
                    <Input
                      type="number"
                      value={s.pro_plan_price_yearly}
                      onChange={(e) => updateField('pro_plan_price_yearly', e.target.value)}
                      className="pl-7 bg-[oklch(0.14_0.02_250)] border-[oklch(0.28_0.03_250)] text-white"
                    />
                  </div>
                </FieldRow>
              </div>
              {s.pro_plan_price_monthly && s.pro_plan_price_yearly && (
                <p className="text-[10px] text-emerald-400/70 mt-2">
                  💡 Yearly plan saves tenants{' '}
                  ₹
                  {(
                    Number(s.pro_plan_price_monthly) * 12 -
                    Number(s.pro_plan_price_yearly)
            ...
            <div>
              <div className="flex items-center gap-2 mb-1">
                <Label className="text-white/70 text-sm">Pro Plan Features</Label>
              </div>
              <p className="text-[11px] text-white/40 mb-3">
                Select features included with the Pro plan
              </p>
              <div className="grid grid-cols-2 gap-2">
                {PRO_FEATURE_OPTIONS.map((feat) => (
                  ...
                  >
                    <Checkbox
                      ...
                      onCheckedChange={() => toggleProFeature(feat.id)}
                      className="data-[state=checked]:bg-purple-600 data-[state=checked]:border-purple-600"
                    />
                    <span
                      className={cn(
                        'text-xs',
                        proFeatures.includes(feat.id) ? 'text-white/90' : 'text-white/50',
                      )}
                    >
                      {feat.label}
                    </span>
